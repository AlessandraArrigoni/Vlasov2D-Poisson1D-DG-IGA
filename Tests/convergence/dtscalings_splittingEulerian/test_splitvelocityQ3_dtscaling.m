...
Dati= struct(  'testname',         'conv_dtscaling',...
               'domain',           [-pi,pi; -4,4],...              
...
filenames = Splitting2orderA_conv_scaling(Dati);
...
subplot(1,3,2); 
loglog(hh, 0.6*hh.^(femregion.degree-1),'k-.','LineWidth',1); 
loglog(hh, 3*hh.^(femregion.degree+1),'k--','LineWidth',1); 
...
subplot(1,3,3);
loglog(hh, 0.5*hh.^(femregion.degree-1),'k-.','LineWidth',1); 
loglog(hh, 5*hh.^(femregion.degree+1),'k--','LineWidth',1); 
