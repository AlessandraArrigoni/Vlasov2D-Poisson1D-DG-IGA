%% Splitting II order starting from transport in v (advection = electric field)
...
filenames = Splitting2orderB_conv_scaling(Data);

save('filenamesBQ3','filenames');
...
for i = 1:length(Data.scalings)
    subplot(1,3,1)
    loglog(hh, vlasov(:,i),'-','Marker',markers(i),'Color',colors(i,:),'LineWidth',1,'MarkerSize',6)
    hold on; grid on
    leg1{i} = ['CFL = ' num2str(4/Data.scalings(i),'%.2f')];
    
    subplot(1,3,2)
    loglog(hh, poisson(:,i),'-','Marker',markers(i),'Color',colors(i,:), 'LineWidth',1,'MarkerSize',6)
    hold on; grid on
    leg2{i} = ['CFL = ' num2str(4/Data.scalings(i),'%.2f')];
    
    subplot(1,3,3)
    loglog(hh, electric(:,i),'-','Marker',markers(i),'Color',colors(i,:), 'LineWidth',1,'MarkerSize',6)
    hold on; grid on
...
% Q3
subplot(1,3,1); 
loglog(hh, 5*hh.^(femregion.degree-1 ),'k-.','LineWidth',1); 
loglog(hh, 100*hh.^(femregion.degree+1),'k--','LineWidth',1); 
leg1{i+1} = ['$O(h^' num2str(femregion.degree-1) ')$'];
leg1{i+2} = ['$O(h^' num2str(femregion.degree+1) ')$'];
...
subplot(1,3,2); 
loglog(hh, hh.^(femregion.degree-1),'k-.','LineWidth',1); 
loglog(hh, 5*hh.^(femregion.degree+1),'k--','LineWidth',1); 
leg2{i+1} = ['$O(h^' num2str(femregion.degree-1) ')$'];
leg2{i+2} = ['$O(h^' num2str(femregion.degree+1) ')$'];
...
leg3{i+2} = ['$O(h^' num2str(femregion.degree+1) ')$'];
