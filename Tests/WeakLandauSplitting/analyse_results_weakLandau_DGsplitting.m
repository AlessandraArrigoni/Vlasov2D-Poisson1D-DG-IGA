...
semilogy(tt(1:floor(length(tt)/2)), exp(coef(1)*tt(1:floor(length(tt)/2))+coef(2)),'r-.','LineWidth',1.3);
...
%% SAVE RESULTS TO VTK
filename = ['landau_splittingA_' Data.fem '_ref' num2str(ref) '_t'];
tempname = [filename '0'];
[basis_plot, coord_plot] = write_solDG_to_vtk(tempname, Data, femregion, ff.first, [3,8], 1);

for j = 1:10
    tempname = [filename num2str(j)];
    tempsol = ff.(['n' num2str(j)]);
    
    [basis_plot, coord_plot] = write_solDG_to_vtk(tempname,Data, femregion, tempsol, [3,8],1, basis_plot, coord_plot);
end
...
for ref = 6:7
    load(['smallWeakLandau_splittingA_Q2_ref' num2str(ref) '.mat']);
    tt = linspace(0, Data.Tend, length(diagnostics.d_energy_rel));
...
colors{6} = 'blue'; colors{7} = 'black';
for ref = 6:7
    load(['smallWeakLandau_splittingA_Q3_ref' num2str(ref) '.mat']);
    tt = linspace(0, Data.Tend, length(diagnostics.d_energy_rel));
