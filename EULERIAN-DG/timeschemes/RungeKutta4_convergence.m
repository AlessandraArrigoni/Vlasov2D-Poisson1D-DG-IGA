function [filenames] = RungeKutta4_convergence(Data)
...
% values at the final time. 
    ...
    for ref = Data.nref % Loop over refinements
        ...
        dt = region.h/Data.dt_scaling; % time discretization parameter (h/4 per Q2, h/6 per Q3)
        ...
        for t = 0 : dt : Data.Tend-dt % Time loop 
            ...
            fprintf('\n--------STEP 1 for t = %d \n',t);
            Data.time = t;
            % Compute rhs and term depending on E for Vlasov equation
            [Le, source1] = matrix_rhs_onlyE(femregion, neighbour, Data ,basis, E_old);       
            LDG1 = Lv + Le;

            % FIRST step in Runge Kutta 4
            kappa1 = dt*Minv*(-LDG1*f_old + source1); 
            ...
            Data.time = t+0.5*dt;
            % Compute rhs and term depending on E for Vlasov equation
            [Le, source2] = matrix_rhs_onlyE(femregion, neighbour, Data ,basis, E_temp);   
            LDG2 = Lv + Le;

            % SECOND step in Runge Kutta 4
            kappa2 = dt*Minv*(-LDG2*(f_old + 0.5*kappa1) + source2);
            ...
            Data.time = t+0.5*dt;
            % Compute rhs and term depending on E for Vlasov equation
            [Le, source3] = matrix_rhs_onlyE(femregion, neighbour, Data ,basis, E_temp);   
            LDG3 = Le + Lv;

            % THIRD step in Runge Kutta 4
            kappa3 = dt*Minv*(-LDG3*(f_old + 0.5*kappa2) + source3);
            ...
            Data.time = t+dt;
            % Compute rhs and term depending on E for Vlasov equation
            [Le, source4] = matrix_rhs_onlyE(femregion, neighbour, Data ,basis, E_temp);   
            LDG4 = Lv + Le;

            % FOURTH step in Runge Kutta 4
            kappa4 = dt*Minv*(-LDG4*(f_old + kappa3) + source4);
            ...
            if(mod(count, 0.25/dt)==0)
                ...
                save(filename, 'ff','phiphi','L2normE','d_energy','d_charge','d_L2normVlasov','-append');
            end
        ...
        if Data.computeErrors
            ...
            vlasov_L2 = err_vlasov.E_L2;
            [poisson_l2, electric_l2] = errorsL2_field_gradient (space_temp, msh_temp, phi_new, @(x) Data.uex_poisson(x,Data.Tend), @(x) Data.gradex_poisson(x,Data.Tend));
    
            save(filename, 'vlasov_L2','poisson_l2','electric_l2','-append');    
        end
